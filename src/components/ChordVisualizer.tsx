...
export const CHORD_LIBRARY: Record<string, ChordData> = {
  ...
  'D#': { guitar: [-1, 6, 8, 8, 8, 6], piano: [3, 7, 10] },
  ...
  return (
    <AnimatePresence>
      {chord && (
        ...
        >
          <div 
            className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-3xl sm:rounded-[2.5rem] w-full max-w-sm overflow-hidden shadow-2xl transition-colors"
            onClick={e => e.stopPropagation()}
          >
            <div className="p-6 sm:p-8 flex flex-col items-center">
              <div className="w-full flex justify-between items-start mb-4 sm:mb-6">
                <div className="flex flex-col">
                  <h2 className="text-4xl sm:text-5xl font-black text-zinc-900 dark:text-white tracking-tighter transition-colors">{chord}</h2>
                  <p className="text-zinc-500 text-[10px] sm:text-xs font-bold uppercase tracking-widest mt-1">Diagrama de Acorde</p>
                </div>
                <button 
                  onClick={onClose}
                  className="p-2 bg-zinc-100 dark:bg-zinc-800 text-zinc-500 dark:text-zinc-400 rounded-full hover:text-black dark:hover:text-white transition-all shadow-sm"
                >
...
export function GuitarDiagram({ frets, size = "normal" }: { frets: number[], size?: "normal" | "small" }) {
  ...
  // Barre detection
  // A barre is detected if the same minimum fret (>0) is used on more than one string
  const fingersOnFrets = frets.filter(f => f > 0);
  const minFretUsed = fingersOnFrets.length > 0 ? Math.min(...fingersOnFrets) : 0;
  const barreStrings = frets.map((f, i) => f === minFretUsed ? i : -1).filter(i => i !== -1);
  const hasBarre = barreStrings.length > 1;

  // To determine the barre span, find the first and last string that uses the barre fret 
  // OR a higher fret (since a barre covers strings even if higher frets override it)
  let barreStart = -1;
  let barreEnd = -1;
  
  if (hasBarre) {
    for (let i = 0; i < strings; i++) {
      if (frets[i] >= minFretUsed) {
        if (barreStart === -1) barreStart = i;
        barreEnd = i;
      }
    }
  }
  ...
  return (
    <div className={cn("relative px-1", size === "small" ? "pt-2" : "pt-6 px-4")}>
      ...
      <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`}>
        {/* Nut */}
        <line x1={paddingX} y1={nutY} x2={paddingX + 5 * stringSpacing} y2={nutY} className={cn("transition-colors", startFret === 1 ? "stroke-zinc-900 dark:stroke-white" : "stroke-zinc-300 dark:stroke-zinc-800")} strokeWidth={size === "small" ? "2" : "4"} />
        
        {/* Frets */}
        {[1, 2, 3, 4, 5].map(i => (
          <line key={i} x1={paddingX} y1={nutY + i * fretSpacing} x2={paddingX + 5 * stringSpacing} y2={nutY + i * fretSpacing} className="stroke-zinc-200 dark:stroke-zinc-800 transition-colors" strokeWidth={size === "small" ? "1" : "2"} />
        ))}
        
        {/* Strings */}
        {[0, 1, 2, 3, 4, 5].map(i => (
          <line key={i} x1={paddingX + i * stringSpacing} y1={nutY} x2={paddingX + i * stringSpacing} y2={nutY + 5 * fretSpacing} className="stroke-zinc-200 dark:stroke-zinc-800 transition-colors" strokeWidth="1" />
        ))}

        {/* Barre (Pestana) */}
        {hasBarre && (() => {
          const relativeFret = minFretUsed - startFret + 1;
          if (relativeFret < 1 || relativeFret > 5) return null;
          
          const x1 = paddingX + barreStart * stringSpacing;
          const x2 = paddingX + barreEnd * stringSpacing;
          const y = nutY + relativeFret * fretSpacing - (fretSpacing / 2);
          const barreWidth = size === "small" ? 4 : 12;
          
          return (
            <rect 
              x={x1 - barreWidth/2} 
              y={y - barreWidth/2} 
              width={(barreEnd - barreStart) * stringSpacing + barreWidth} 
              height={barreWidth} 
              rx={barreWidth/2}
              fill="#38bdf8" 
            />
        ...
        {frets.map((fret, stringIdx) => {
          ...
          // Skip dots that are covered by a barre at the same position
          if (hasBarre && fret === minFretUsed) return null;
...
export function PianoDiagram({ activeKeys, size = "normal" }: { activeKeys: number[], size?: "normal" | "small" }) {
  ...
  const width = size === "small" ? 80 : (typeof window !== 'undefined' && window.innerWidth < 400 ? 240 : 280);
  const height = size === "small" ? 24 : (typeof window !== 'undefined' && window.innerWidth < 400 ? 70 : 80);
  const whiteKeyWidth = size === "small" ? 6 : (typeof window !== 'undefined' && window.innerWidth < 400 ? 16 : 19);
  const blackKeyWidth = size === "small" ? 4 : (typeof window !== 'undefined' && window.innerWidth < 400 ? 12 : 14);
  const blackKeyHeight = size === "small" ? 14 : (typeof window !== 'undefined' && window.innerWidth < 400 ? 40 : 45);
  const spacing = size === "small" ? 5.71 : (typeof window !== 'undefined' && window.innerWidth < 400 ? 17.14 : 20);

  return (
    <div className={cn("bg-zinc-100 dark:bg-zinc-800 p-1 rounded-lg overflow-hidden transition-colors shadow-inner", size === "small" ? "p-0.5" : "p-1")}>
      <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`}>
        {/* White Keys */}
        {Array.from({ length: 14 }).map((_, i) => {
          ...
          return (
            <rect 
              ...
              width={whiteKeyWidth} 
              height={height} 
              fill={isActive ? "#38bdf8" : "white"} 
              rx={size === "small" ? "1" : "2"}
              className="stroke-zinc-200 dark:stroke-transparent transition-colors"
            />
          );
        })}
